...
namespace ecs {
    ...
    static void register_systems(flecs::world &world, ECSWorld* ecs_world) {
        // 移动系统
        world.system<Position, const Velocity>("MoveSystem")
            .kind(flecs::OnUpdate)
            .each([&world](Position& p, const Velocity& v) {
        ...
        /* 子弹 */ {
            ...
            world.system<const Position, const Velocity, const Rotation, const Team, BulletCollision, const SizeValue,
                const BulletTypeComp, const BulletDamage, const BulletOrigin*>("BulletCollisionSystem")
                .kind(flecs::OnUpdate)
                .without<PointBullet>()
                //.multi_threaded(false)
                .each([&world, &ecs_world](flecs::entity e, const Position& p, const Velocity& v, const Rotation& r, const Team& t,
                    BulletCollision& bc, const SizeValue& s, const BulletTypeComp& bt, const BulletDamage& bd, const BulletOrigin* origin)
                {
                    ...
                    if (bt.collide_block && !hit_unit) {
                        ...
                        if (block_v.get_type() != Variant::NIL && static_cast<int>(block_v.get("team")) != t.value) {
                            ...
                            for (int64_t cid : bc.collided) {
                                if (cid == id) { already_hit = true; break; }
                            }
                            if (!already_hit) {
                                bc.collided.push_back(id);
